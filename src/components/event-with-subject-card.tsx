...
export function EventWithSubjectCard({ event, onDelete, onToggleComplete }: EventWithSubjectCardProps) {
  ...
  // Determinar a rota para navegação
  const handleNavigation = () => {
    if (event.subject_id) {
      router.push(`/dashboard/subjects/${event.subject_id}/events/${event.id}`)
    } else {
      // Para eventos gerais, navegar para a rota específica
      router.push(`/dashboard/events/${event.id}`)
    }
  }

  // Definir cor e nome da matéria para eventos gerais
  const subjectColor = event.subject?.color || "#808080" // Cor cinza padrão para eventos gerais
  const subjectName = event.subject?.name || "Geral"
